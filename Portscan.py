        ...
        try:
            ...
            # En esta opcion el usuario ingresa la IP        
                ...
                if path.exists("LastIP.txt"):
                    Lastipfile=open("LastIP.txt","r")
                    ip=Lastipfile.readline()
                    break
                else:
                    print("El archivo no existe , favor de correr modulo Shodan")    
                    input()   
...
def genreport(closePort,openPort,ip):
    try: # Exepciones para el manejo de archivo
        ...
        print(f"Archivo guardado como: {base_filename}({counter-1})")
        print("Se ejecuto Escaneo de puertos en la fecha:",now)
        print("Hash del arcvhio: ",hash)
        print("Presione enter para continuar ")
        input()
        system("cls") #Limpiando pantalla 
