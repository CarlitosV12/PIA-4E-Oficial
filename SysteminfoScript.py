...
def memory():
    ...
    filename = "SysInfo"
    total_body = ""
    total_body += "Datos sobre la RAM\n\n"
    total_body += f"Total de memoria RAM: {total_memory} GB\n"
    total_body += f"Uso de la memoria RAM: {percent_memory}%\n"
    total_body += f"Memoria disponible: {avememory} GB\n"
    total_body += f"Memoria en uso: {used_memory} GB\n"
    total_body += f"Memoria libre: {free_memory} GB\n\n"
        ...
        try:
            ...
            if option=="y": # Si acepta empezara el proceso
                ...
                for process in processlist[:10]:
                    ...
                    body_cicle = f"\n{x}-Nombre del proceso : {process['name']}\n"
                    body_cicle2 = f"Ram utilizada : {process_ram} MB\n"
                    body_cicle3 = f"Porcentaje :{percent_ram_process} %\n\n"
                    total_body += body_cicle
                    total_body += body_cicle2
                    total_body += body_cicle3
            ...
            print("Dato invalido")
    print("Tarea que se ejecutó: Información de la RAM\n")
...
def disk():
    ...
    filename = "SysInfo"
    total_body = ""
    total_body += "Información del disco:\n"
    ...
    for partition in disk:
        try:
            ...
            total_body += f"Partición: {partition.device}\n"
            #Imprimimos punto de montaje , sistema de archivos
            # Espacio total , usado y libre y el porcentaje 
            total_body += f"  Punto de montaje: {partition.mountpoint}\n"
            total_body += f"  Tipo de sistema de archivos: {partition.fstype}\n"
            total_body += f"  Espacio total: {disk_usaged.total / (1024**3):.2f} GB\n"
            total_body += f"  Espacio usado: {disk_usaged.used / (1024**3):.2f} GB\n"
            total_body += f"  Espacio libre: {disk_usaged.free / (1024**3):.2f} GB\n"
            total_body += f"  Porcentaje de uso: {disk_usaged.percent}%\n\n"
...
los procesos, no el espacio que ocupan en el almacenamiento.
                ...
                for name, io in disk_process[:10]:
                    ...
                    body_cicle = f"\n{i}-Proceso: {name} \n"
                    body_cicle2 = f"Uso: {io/(1024**3):.2f} GB\n"
                    total_body += body_cicle
                    total_body += body_cicle2
    ...
    print(Fore.RED+"Presione enter para continuar ")
    input()
    print("Tarea que se ejecutó: Información del disco\n")
    write_arc(filename,total_body)
    print("Presione enter para continuar")
    input()
    return 0
...
def cpu_info():
    ...
    try:
        ...
        filename = "SysInfo"
        total_body = ""
        total_body += f"Frecuencia máxima del CPU: {frecuency.max:.2f} MHz\n"
        total_body += f"Frecuencia actual del CPU: {frecuency.current:.2f} MHz\n"
        total_body += f"Número de núcleos físicos: {fisic_cores}\n"
        total_body += f"Número de núcleos lógicos: {total_cores}\n"
        total_body += f"Uso total del CPU: {total_cpu}%\n\n"
        ...
        total_body += f"\n\nEstadísticas del CPU:\n\n"
        # Numero de context switches desde el boot
        total_body += f"Context Switches: {statsCPU.ctx_switches}\n"
        # Numero de interrumpciones desde el boot
        total_body += f"Interrupciones: {statsCPU.interrupts}\n"
        # Numero de llamadas al sistema desde el boot
        total_body += f"Llamadas al sistema: {statsCPU.syscalls}\n"
        # Numero de interrumpciones de software desde el boot
        total_body += f"Interrumpciones de software: {statsCPU.soft_interrupts}\n\n"
    ...
    print("Tarea que se ejecutó: Información de la CPU\n")
    write_arc(filename,total_body)
    print("Presione enter para continuar")
    input()
    return 0
